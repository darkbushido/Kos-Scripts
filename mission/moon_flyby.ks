...
function free_return_correction {
  ...
  local data is list(0,0,0).
  for step in list(100,10,1,0.1) {set data to hc["seek"](data, transfit["cor_per_fit"](ct, kerbin, 35000), step).}
  local nn to nextnode.
...
function finish {
  ...
  seq:add(free_return_correction@). seqn:add("free_return_correction").
  seq:add(collect_science@). seqn:add("collect_science").
