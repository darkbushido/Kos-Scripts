...
local hollclimb is import("lib/hillclimb.ks").
local PARKING_ALTITUDE is BODY:ATM:HEIGHT + 10000.
local TARGET_ALTITUDE is 30000.
...
function mission_definition {
  ...
  function coast_to_atm {
    if alt:radar > body:atm:height {
      ...
      if ev:haskey("AutoStage")
        ev:remove("AutoStage").
      print ship_utils["stageDV"]().
      if ship_utils["stageDV"]() < 200
        stage.
      wait 0.
      panels on.
      next().
  ...
  function perform_transfer {
    hillclimb["seek"](TARGET_BODY, TARGET_ALTITUDE).
    node_exec["exec"](true).
    next().
  }
  // seq:add(circularize@).
