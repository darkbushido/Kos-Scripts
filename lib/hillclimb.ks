{
  ...
  function best_neighbor {
    ...
    for neighbor in neighbors(d, ss) {
      local neighbor_key to "".
      for n in neighbor { set neighbor_key to neighbor_key + n + "-". } local ch to false.
      if fitness_lookup:haskey(neighbor_key) { set ch to true. set fitness to fitness_lookup[neighbor_key]. }
