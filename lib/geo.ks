...
function geo_posAt {
	...
	//debug stuff - feel free to remove
	local markGeo is vecs_add(geopos:position,geoposUP:normalized * 100000,rgb(0,1,1),"GeoPos (" + round(geopos:lat,2) + "," + round(geopos:lng,2) + ")").
	local markPos is vecs_add(newpos,newposUP:normalized * 100000,rgb(1,0,0),"Newpos").

	set countdown to time:seconds + t. ag1 off. local shippos is body:position.
	until time:seconds > countdown or ag1 {
		print round(countdown - time:seconds) + "       " at (0,1).
		set vecs[markGeo]:start to geopos:position.
		set vecs[markGeo]:vec to (geopos:position - body:position):normalized * 100000.
		set vecs[markPos]:start to newpos + body:position - shippos.
	}
	//end of debug
